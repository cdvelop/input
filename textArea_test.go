...
var (
	modelTextArea = input.TextArea("", "", 0, 0)
	...
	dataTextArea = map[string]struct {
		...
		"letra ñ permitida? parentecis y $": {"soy ñato o Ñato (aqui) costo $10000.", false, true},
		"solo texto y espacio?":             {"hola esto es una prueba", false, true},
		"texto y puntuacion?":               {"hola: esto es una prueba", false, true},
		"texto y puntuacion y coma?":        {"hola: esto,true, es una prueba", false, true},
		"4 caracteres?":                     {" .s5", false, true},
		"sin data permitido?":               {"", false, false},
		"# permitido?":                      {"# son", false, false},
		"¿ ? permitido?":                    {" ¿ si ?", false, false},
		"tíldes si?":                        {" mí tílde", false, true},
		"1 carácter?":                       {"1", false, false},
		"nombre correcto?":                  {"Dr. Pato Gome", false, true},
		"solo espacio en blanco?":           {" ", false, false},
		"textto largo correcto?":            {`IRRITACION EN PIEL DE ROSTRO. ALERGIAS NO. CIRUGIAS NO. ACTUAL TTO CON ISOTRETINOINA 10MG - ENERO 2022. EN TTO ACTUAL CON VIT D. EXAMEN DE LAB 20-12-2022. SIN OTROS ANTECEDENTES`, false, true},
	}
)
...
func Test_GoodInputTextArea(t *testing.T) {
	for _, data := range modelTextArea.TestData.GoodTestData("", "", true) {
